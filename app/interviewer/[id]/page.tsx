			...
			<div className="space-y-5">
				...
				<div className="flex flex-wrap gap-4 text-xs text-[var(--foreground-muted)]">
					<span className="flex items-center gap-1">
						<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M5 8l7 4 7-4" /><path d="M5 12l7 4 7-4" /><path d="M5 16l7 4 7-4" /></svg>
						Email: {interview.profile?.email || "n/a"}
					</span>
					<span className="flex items-center gap-1">
						<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M3 5a2 2 0 012-2h1.28a1 1 0 01.95.684l1.518 4.554a1 1 0 01-.502 1.21l-1.257.628a11.042 11.042 0 006.105 6.105l.628-1.257a1 1 0 011.21-.502l4.554 1.518a1 1 0 01.684.95V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
						Phone: {interview.profile?.phone || "n/a"}
					</span>
					<span className="opacity-70 font-mono">Session: {interview.sessionId}</span>
				</div>
				<div className="flex gap-2 flex-wrap text-[10px]">
					<Badge variant="secondary" className="bg-[var(--accent)]/15 border-[var(--accent)]/30 text-[var(--accent)] font-medium">
						Final Score {interview.finalScore}
					</Badge>
					<Badge variant="secondary" className="bg-white/10 border-white/10 text-[var(--foreground-muted)]">
			...
			<section className="space-y-4">
				...
				<div className="space-y-6">
					{interview.questions.map((q: Record<string, unknown>) => (
						<div key={q.id as string} className="p-6 rounded-xl bg-gradient-to-br from-white/8 to-white/3 border border-white/10 space-y-4">
